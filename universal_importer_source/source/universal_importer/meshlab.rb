  ...
  module MeshLab
    ...
    def self.exe
      ...
      if UniversalImporter.platform == :platform_osx
        ...
        return File.join(File.dirname(__FILE__), 'Applications', 'MeshLab', 'Mac', 'MacOS', 'meshlabserver')

      elsif UniversalImporter.platform == :platform_win

        return File.join(File.dirname(__FILE__), 'Applications', 'MeshLab', 'Win', 'meshlabserver.exe')

      else
        raise StandardError.new('Unsupported platform: ' + UniversalImporter.platform.to_s)
      end
    ...
    def self.apply_script(working_dir, in_filename, out_filename, mlx_filename, log_filename)
      ...
      if UniversalImporter.platform == :platform_win
