...
module UniversalImporter
  ...
  class Import
    ...
    def create_link_to_source_file
      ...
      # For network paths (UNC paths starting with //), use file copy instead of hard link
      if @source_dir =~ /^\/\//
        # Network path detected - use file copy instead of hard link
        File.open(@source_file_path, 'rb') do |source|
          File.open(source_link_path, 'wb') do |dest|
            dest.write(source.read)
          end
        end
      else
        # Local path - use hard link as before
    ...
    def convert_source_link_to_intermediate
      ...
      if File.exists?(@inter_mtl_file_path)
        ...
        File.open(@inter_mtl_file_path, 'w') { |f| f.write(inter_mtl) }
      end
    ...
    def fix_3mf_embedded_tex_in_inter_mtl
      ...
      if textures_extracted
        ...
        # File.write replacement for SketchUp 8 compatibility
        File.open(@inter_mtl_file_path, 'w') { |f| f.write(intermediate_mtl.to_s) }
      end
    end
    ...
    def fix_external_tex_in_inter_mtl
      ...
      if !external_texture_paths.empty?
        ...
        source_parent_dir.gsub!('\\', '/') if UniversalImporter.platform == :platform_win
    ...
    def ask_for_missing_tex_in_inter_mtl
      ...
      mtl.materials.each do |material_name, material|
        ...
        if !material[:diffuse_texture]
          ...
          unless FS.create_hard_link(texture_link_or_copy_path, texture_path)
            ...
            # FileUtils.copy replacement for SketchUp 8 compatibility
            File.open(texture_link_or_copy_path, 'wb') do |dest|
              File.open(texture_path, 'rb') do |src|
                dest.write(src.read)
              end
            end
          end
      ...
      if user_provided_texture
        # In this case, an update of the intermediate MTL file is required.
        # File.write replacement for SketchUp 8 compatibility
        File.open(@inter_mtl_file_path, 'w') { |f| f.write(mtl.to_s) }
      end
    end
    ...
    def apply_poly_reduction_to_inter_obj
      ...
      # File.write replacement for SketchUp 8 compatibility
      File.open(File.join(@source_dir, 'uir-poly_reduction.mlx'), 'w') { |f| f.write(mlx) }
    ...
    def delete_temp_files
      ...
      if File.directory?(@temp_textures_dir)
        # Recursively delete directory contents
        Dir.foreach(@temp_textures_dir) do |file|
          next if file == '.' || file == '..'
          file_path = File.join(@temp_textures_dir, file)
        ...
        Dir.delete(@temp_textures_dir)
      end

      temp_files_pattern = @source_dir

      # Dir.glob() only uses "/" to separate dirs, so we fix that (on Windows).
      temp_files_pattern.gsub!('\\', '/') if UniversalImporter.platform == :platform_win
